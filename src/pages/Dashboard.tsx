...
export default function Dashboard() {
  ...
  const [latestSleepScore, setLatestSleepScore] = useState<number>(0);
  ...
  useEffect(() => {
    if (profile && healthLogs.length > 0) {
      runAiAnalysis();
    }
  ...
  const runAiAnalysis = () => {
    ...
    const avgSleepScore =
      sleepEntries.length > 0
        ? sleepEntries.reduce((acc, s) => acc + (s.sleep_score || 0), 0) / sleepEntries.length
        : 70;

    const analysis = analyzeHealth({
      symptoms: latestLog?.symptoms || [],
      severity: latestLog?.severity || 0,
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container px-4 py-6">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {/* AI Health Score */}
          <div className="card-medical p-6 md:col-span-2 lg:col-span-1">
            ...
            <div className="flex flex-col items-center py-4">
              <HealthScoreGauge score={aiAnalysis?.healthRiskScore || 0} size="lg" />
                ...
                <div className="mt-3 flex gap-1">
                  {sleepEntries.slice(0, 7).reverse().map((entry, idx) => (
                    <div
                      ...
                      title={`${entry.logged_date}: ${entry.sleep_score}/100`}
                    />
                  ))}
                </div>
          ...
          <div className="card-medical p-6 md:col-span-2 lg:col-span-2">
        ...
        <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Heart className="h-5 w-5 text-destructive" />I Feel Uneasy
            </DialogTitle>
            <DialogDescription>
          ...
          {aiAnalysis && (
            <div className="space-y-6 py-4">
              <div className="flex justify-center">
                <HealthScoreGauge score={aiAnalysis.healthRiskScore} size="md" />
              </div>
